...
view =
    ...
    , content =
        div []
            ...
            , p [] [ text "\u{3000}鱗翅目はその生物的な特性から21上科に分けられ，このうちの「ゴルフクラブ状の触角を持つアゲハチョウ上科 Papilionoidea 」と「触角の端部は膨大しているがその先端がかぎ状にとがるセセリチョウ上科 Hesperioidea」 の2上科が蝶に当たる。（図１参照）\u{3000}現在知られている鱗翅目は約14万種であるが、このうちチョウ類は約1万8000種に過ぎない。" ]
            , p [] [ text "\u{3000}これはチョウに限ったことではないが、植物に依存する昆虫はその植物の生育する環境や気象条件に適応した結果独自の特徴をもち、上述の上科にシート２に示す科の分化が、さらに細かく後述の属のレベルの分化が起こったと考えらている。" ]
            , hr [ class "hr" ] []
            ...
            , p [] [ text "\u{3000}これに基づいて、種の学名は、その種が属している属名(名詞で冒頭の字は大文字)に、種の性質を示す種小名(形容詞で冒頭の字は小文字)をつけて、２語で表現している。なお、属名も種小名もすべてラテン語ないしラテン語の形式を使用している。" ]
